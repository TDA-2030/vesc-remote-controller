...
{
	uint16_t retry=0;		 	
	while(!LL_SPI_IsActiveFlag_TXE(SPI1))		//等待发送区空	
	{
		retry++;
		if(retry>=0XFFFE)return 0; 	//超时退出
	}
	LL_SPI_TransmitData8(SPI1, indata);	 	  		//发送一个byte 
	retry=0;
	while(!LL_SPI_IsActiveFlag_RXNE(SPI1)) 		//等待接收完一个byte  
	{
		retry++;
		if(retry>=0XFFFE)return 0;	//超时退出
	}	  						    
...
void RF24L01_Init( void )
{
    ...
    NRF24L01_Write_Reg(RX_PW_P0,NRF_FIXED_PACKET_LEN); 	//固定数据长度
    ...
    NRF24L01_Write_Reg( SETUP_RETR, ARD_4000US |
                        ( NRF_REPEAT_CNT & 0x0F ) );         	//重复等待时间 250us
...
{
	RF24L01_SET_CE_LOW( );	  
  	NRF24L01_Write_Buf(NRF_WRITE_REG+RX_ADDR_P0,(uint8_t*)RX_ADDRESS_0,5);//写RX节点地址
